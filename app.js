...
function renderApp() {
    ...
    console.log(`Attempting to render page: ${currentPage}`);
    ...
    switch (currentPage) {
        ...
        default:
            ...
            if (taskExists) {
                ...
                console.error(`Invalid currentPage ID: ${currentPage}. Redirecting to map.`);
                currentPage = 'map'; // Reset to map if ID is invalid
                renderMapView(rootDiv);
            }
            break;
    }
...
function renderMapView(container) {
    ...
    mapLevelNodes.forEach(mapNode => {
        ...
        phaseHeader.className = 'retro-panel-heading mb-2';
        ...
        phaseDescription.className = 'retro-panel-body-text mb-3';
        ...
        if (expandedPhases[mapNode.id]) { // Use mapNode.id
            ...
            if (subTaskChildren.length > 0) {
                ...
                subTaskChildren.forEach(subTask => { // Iterate over subTaskChildren
                    ...
                    subTaskName.className = 'retro-sub-task-heading';
...
function renderTaskDetailView(container, taskId) {
    ...
    function createDetailSection(title, content) {
        ...
        if (Array.isArray(content)) {
            ...
            p.textContent = content.toString(); // Ensure content is always a string
