...
function renderMapView(container) {
    ...
    mapLevelNodes.forEach(mapNode => { // Changed from phase to mapNode for clarity
        const phaseElement = document.createElement('div'); // Keep class names for styling consistency
        ...
        if (expandedPhases[mapNode.id]) { // Use mapNode.id
            ...
            if (subTaskChildren.length > 0) {
                ...
                subTaskChildren.forEach(subTask => { // Iterate over subTaskChildren
                    ...
                    subTaskElement.className = 'retro-panel bg-purple-700 border border-purple-400 rounded p-3 mb-2 shadow-md hover:bg-purple-600'; // Existing class for sub-items
    ...
    if (selectedTask) {
        ...
        if (task) {
            ...
            // Basic styling for the panel, could be fixed/absolute positioned
            detailPanel.className = 'fixed bottom-0 left-0 right-0 bg-gray-900 border-t-4 border-yellow-400 p-6 shadow-2xl z-50 max-h-[40vh] overflow-y-auto'; // Changed max-h-1/3 to max-h-[40vh]
